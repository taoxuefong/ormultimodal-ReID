...
class CLIP2ReID(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.args.only_sketch:
            ...
            ret.update({'itc_loss':(objectives.compute_itc(i_feats, si_feats, logit_scale))*self.args.cmm_loss_weight})
        elif self.args.only_text:
            i_feats = image_feats[:, 0, :].float()
            t_feats = text_feats[torch.arange(text_feats.shape[0]), caption_ids.argmax(dim=-1)].float() #[64, 512]
            ret.update({'itc_loss':(objectives.compute_itc(i_feats, t_feats, logit_scale))*self.args.cmm_loss_weight})
        ...
        #     ret.update({'itc_loss':(objectives.compute_itc(i_feats, f_feats, logit_scale))*self.args.cmm_loss_weight})
            ...
            if self.args.fusion_way in ['add', 'weight add', 'cross attention', 'parameter add', 'concat', 'global concat', 'cross attention text', 'cross attention sketch', 'concat transformer']:
                f_feats = self.fusion_layer(text_feats, simage_feats, caption_ids, pa=self.args.pa, way=self.args.fusion_way)
                i_feats = image_feats[:, 0, :].float()
                si_feats = simage_feats[:, 0, :].float()
                t_feats = text_feats[torch.arange(text_feats.shape[0]), caption_ids.argmax(dim=-1)].float() #[64, 512]
                ...
                si_feats = simage_feats[:, 0, :].float()
                t_feats = text_feats[torch.arange(text_feats.shape[0]), caption_ids.argmax(dim=-1)].float() #[64, 512]
                ret.update({'itc_loss':(objectives.compute_itc(i_feats, t_feats, logit_scale)+objectives.compute_itc(i_feats, si_feats, logit_scale))*self.args.cmm_loss_weight})
...
def build_model(args, num_classes=11003):
    if args.dataset_name == 'PRCV':
        model = build_multimodal_model(args, num_classes)
    else:
