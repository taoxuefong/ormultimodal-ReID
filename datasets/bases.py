...
class MultiModalDataset(Dataset):
    """Dataset for multi-modal person re-identification"""
    def __init__(self,
                 dataset,
                 transform=None,
                 text_length: int = 77,
    ...
    def __getitem__(self, index):
        pid, image_id, vis_path, nir_path, cp_path, sk_path, caption = self.dataset[index]
...
class MultiModalQueryDataset(Dataset):
    """Dataset for multi-modal queries during inference"""
    def __init__(self,
                 queries,
                 transform=None,
                 text_length: int = 77,
        ...
        self.truncate = truncate
        self.tokenizer = SimpleTokenizer()
        self.base_dir = base_dir or '/data/taoxuefeng/PRCV/val'  # 默认基础目录
    ...
    def __getitem__(self, index):
        ...
        # 添加调试信息
        # print(f"DEBUG: ===== Query {query_idx} =====")
        # print(f"DEBUG: query_type: {query_type}")
        # print(f"DEBUG: Original modalities: {modalities}")
        ...
        # 创建内容和模态的映射
        content_map = {}
        # print(f"DEBUG: Creating content_map...")
        for i, modality in enumerate(modalities):
            content_map[modality] = content[i]
            # print(f"DEBUG: content_map[{modality}] = content[{i}] = {content[i]}")
        
        # print(f"DEBUG: Final content_map: {content_map}")
        
        # Process each modality
        for modality in modalities:
            # print(f"DEBUG: Processing modality: {modality}")
            if modality == 'nir':
                nir_path = content_map[modality]
                # print(f"DEBUG: NIR path: {nir_path}")
                # 安全检查：确保路径是图像文件
                if not nir_path.endswith(('.jpg', '.jpeg', '.png')):
                    # print(f"警告: NIR路径不是图像文件: {nir_path}")
                    nir_img = None
                else:
                    # 构建完整路径
                    full_nir_path = os.path.join(self.base_dir, nir_path)
                    # print(f"DEBUG: Full NIR path: {full_nir_path}")
                    try:
                        nir_img = read_image(full_nir_path)
                        if self.transform:
                            nir_img = self.transform(nir_img)
                    except Exception as e:
                        # print(f"警告: 无法读取NIR图像 {full_nir_path}: {e}")
                        nir_img = None
            elif modality == 'cp':
                cp_path = content_map[modality]
                # print(f"DEBUG: CP path: {cp_path}")
                # 安全检查：确保路径是图像文件
                if not cp_path.endswith(('.jpg', '.jpeg', '.png')):
                    # print(f"警告: CP路径不是图像文件: {cp_path}")
                    cp_img = None
                else:
                    # 构建完整路径
                    full_cp_path = os.path.join(self.base_dir, cp_path)
                    # print(f"DEBUG: Full CP path: {full_cp_path}")
                    try:
                        cp_img = read_image(full_cp_path)
                        if self.transform:
                            cp_img = self.transform(cp_img)
                    except Exception as e:
                        # print(f"警告: 无法读取CP图像 {full_cp_path}: {e}")
                        cp_img = None
            elif modality == 'sk':
                sk_path = content_map[modality]
                # print(f"DEBUG: SK path: {sk_path}")
                # 安全检查：确保路径是图像文件
                if not sk_path.endswith(('.jpg', '.jpeg', '.png')):
                    # print(f"警告: SK路径不是图像文件: {sk_path}")
                    sk_img = None
                else:
                    # 构建完整路径
                    full_sk_path = os.path.join(self.base_dir, sk_path)
                    # print(f"DEBUG: Full SK path: {full_sk_path}")
                    try:
                        sk_img = read_image(full_sk_path)
                        if self.transform:
                            sk_img = self.transform(sk_img)
                    except Exception as e:
                        # print(f"警告: 无法读取SK图像 {full_sk_path}: {e}")
                        sk_img = None
            elif modality == 'text':
                text = content_map[modality]
                # print(f"DEBUG: Text content: {text[:100] if isinstance(text, str) else text}")
                # 确保text是字符串类型
                if not isinstance(text, str):
                    # print(f"警告: 文本模态内容不是字符串: {type(text)}, 值: {text}")
                    text = str(text) if text is not None else ""
                text_tokens = tokenize(text, tokenizer=self.tokenizer, text_length=self.text_length, truncate=self.truncate)

        # 确保所有值都不是None，为None的模态提供占位符
        if nir_img is None:
            # 创建一个占位符图像（3x384x128），不包含batch维度
            nir_img = torch.zeros(3, 384, 128)
        if cp_img is None:
            cp_img = torch.zeros(3, 384, 128)
        if sk_img is None:
            sk_img = torch.zeros(3, 384, 128)
        if text_tokens is None:
            # 创建占位符文本token
            text_tokens = torch.zeros(self.text_length, dtype=torch.long)
        ...
        # print(f"DEBUG: Returning result with keys: {list(ret.keys())}")
        # for key, value in ret.items():
        #     if torch.is_tensor(value):
        #         print(f"DEBUG: {key}: tensor shape {value.shape}")
        #     elif isinstance(value, str):
        #         print(f"DEBUG: {key}: string length {len(value)}")
        #     else:
...
class PRCVTrainDataset(Dataset):
    """专门用于PRCV训练的数据集类，加载所有模态的真实图像"""
    def __init__(self,
                 dataset,
                 transform=None,
                 text_length: int = 77,
    ...
    def __getitem__(self, index):
        ...
        # 如果某个模态没有图像，使用RGB图像作为后备
        if nir_img is None:
            nir_img = vis_img.copy()
        if cp_img is None:
            cp_img = vis_img.copy()
        if sk_img is None:
            sk_img = vis_img.copy()
