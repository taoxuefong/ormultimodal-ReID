...
if __name__ == '__main__':
    ...
    # 设置平衡采样参数
    args.sampler = 'balanced'  # 使用平衡采样器
    args.num_instance = 4      # 每个类别选择4张图片
    
    # 设置更小的log_period，让训练损失值更频繁输出
    args.log_period = 10  # 每10个batch输出一次损失值
    
    # 禁用评估
    args.eval_period = 999999  # 设置一个很大的值，实际上不会进行评估
    ...
    print(f"Sampler: {args.sampler} (balanced sampling)")
    print(f"Num instance per class: {args.num_instance}")
    print(f"Expected classes per batch: {args.batch_size // args.num_instance}")
    print(f"Log period: {args.log_period} (will output loss every {args.log_period} batches)")
    print(f"Evaluation disabled (eval_period={args.eval_period})")
    ...
    logger.info("Training multi-modal ReID model for PRCV dataset (NO EVALUATION)")
